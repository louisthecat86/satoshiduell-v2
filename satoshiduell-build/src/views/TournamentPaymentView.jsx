...
import { updateTournament } from '../services/supabase';

const TournamentPaymentView = ({ tournamentId, amount, onPaymentSuccess, onCancel }) => {
  ...
  useEffect(() => {
    ...
    const fetchInvoice = async () => {
      ...
      const data = await createInvoice(amount, 'SatoshiDuell Tournament Prize Pool');
      ...
      if (data && data.payment_request) {
        ...
        await updateTournament(tournamentId, {
          creator_payment_hash: data.payment_hash,
          creator_payment_request: data.payment_request,
        });
      } else {
  ...
  }, [status, paymentHash, tournamentId, onPaymentSuccess]);
    ...
    <Background>
      <div className="flex flex-col h-full items-center justify-center p-6 text-center">
        <h2 className="text-2xl font-black uppercase italic text-white mb-2">
          {status === 'success' ? t('tournament_payment_success') : t('tournament_payment_title')}
        </h2>
        <p className="text-neutral-400 mb-8 max-w-xs mx-auto">
          {status === 'success' ? t('tournament_payment_redirect') : t('tournament_payment_subtitle')}
        </p>
        ...
        <div className="bg-[#222] px-6 py-3 rounded-full border border-white/10 flex items-center gap-2 shadow-lg">
          <span className="text-orange-500 font-black text-xl">
            {t('tournament_payment_amount')}: {amount} SATS
          </span>
...
export default TournamentPaymentView;
